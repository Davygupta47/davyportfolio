  ...
  return (
    <>
      ...
      <mesh
        ...
        castShadow
        ref={(el) => {
          if (el && rearWheelsRef) rearWheelsRef.current[0] = el;
        }}
      >
      ...
      <mesh
        ...
        castShadow
        ref={(el) => {
          if (el && rearWheelsRef) rearWheelsRef.current[1] = el;
        }}
      >
        <cylinderGeometry args={[1.4, 1.4, 1.8, 16]} />
        <meshStandardMaterial color={0x151515} roughness={0.9} metalness={0} />
      ...
      <sprite
        ...
        scale={[6, 6, 1]}
        ref={(el) => {
          if (el && headlightsRef) headlightsRef.current[0] = el;
        }}
      >
        <spriteMaterial
          map={glowTexture}
      ...
      <sprite
        ...
        scale={[6, 6, 1]}
        ref={(el) => {
          if (el && headlightsRef) headlightsRef.current[1] = el;
        }}
      >
        <spriteMaterial
          map={glowTexture}
...
function ParticleTrail({ targetRef }: { targetRef: React.RefObject<THREE.Group | null> }) {
  ...
  const particles = useMemo(
    () => Array.from({ length: count }, () => ({ pos: new THREE.Vector3(), age: 1000 })),
    [],
  );
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!spawned && p.age > 0.4) {
        ...
        const backOffset = new THREE.Vector3(0, 0.2, -6).applyMatrix4(
          targetRef.current.matrixWorld,
        );
  ...
  useFrame((state) => {
    ...
    if (prevTangent.current.lengthSq() > 0) {
      ...
      carGroupRef.current.rotation.z = THREE.MathUtils.lerp(
        ...
        Math.max(Math.min(targetRoll, 0.15), -0.15),
        0.1,
      );
    }
...
function ResponsiveOrthographicCamera() {
  ...
  return <OrthographicCamera makeDefault position={[0, 200, 0]} zoom={calculatedZoom} />;
}
